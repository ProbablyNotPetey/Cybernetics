...
public class CapabilityEvents {
    ...
    public static <T> void onAttachCapabilitiesEvent(AttachCapabilitiesEvent<Entity> event) {
        if(event.getObject() instanceof Player player) {
            if(!player.getCapability(ModCapabilities.CYBERWARE).isPresent()) {
                event.addCapability(new ResourceLocation(Cybernetics.MOD_ID, "cyberware"), new PlayerCyberwareProvider());
            }
            if(!player.getCapability(ModCapabilities.PLAYER_ENERGY).isPresent()) {
    ...
    public static void onPlayerClonedEvent(PlayerEvent.Clone event) {
        ...
        if(event.isWasDeath()) {
            event.getOriginal().getCapability(ModCapabilities.CYBERWARE).ifPresent(oldStore -> {
                event.getEntity().getCapability(ModCapabilities.CYBERWARE).ifPresent(newStore -> {
                    newStore.copyFrom(oldStore, true);
                });
            });
            event.getOriginal().getCapability(ModCapabilities.PLAYER_ENERGY).ifPresent(oldStore -> {
                event.getEntity().getCapability(ModCapabilities.PLAYER_ENERGY).ifPresent(newStore -> {
